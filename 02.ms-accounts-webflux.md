...
#### ⚙️ Configuración de Auditing — precisión de fechas Java vs PostgreSQL

Para que `@CreatedDate` y `@LastModifiedDate` funcionen, Spring Data necesita que habilitemos la auditoría `R2DBC`
explícitamente con `@EnableR2dbcAuditing`. La forma más simple es hacerlo directamente en la clase principal:
...
Sin embargo, optamos por una **clase de configuración dedicada** porque, además de habilitar la auditoría, necesitamos
resolver un problema de precisión entre `Instant` de Java y `TIMESTAMPTZ` de PostgreSQL:
...
##### 🔍 ¿Por qué es necesario este `DateTimeProvider` personalizado?

**El origen del problema: distinta precisión entre Java y PostgreSQL**

`Instant.now()` en Java captura el tiempo con precisión de **nanosegundos** (9 decimales), que es la resolución máxima
que soporta `java.time`. PostgreSQL, por su parte, almacena `TIMESTAMPTZ` con precisión de **microsegundos**
(6 decimales), que es su resolución máxima nativa.

Esta diferencia de precisión genera una inconsistencia silenciosa:

```
1️⃣ Java genera el timestamp con @CreatedDate:
   2026-06-17T21:32:52.791041800Z   ← 9 decimales (nanosegundos)

2️⃣ PostgreSQL almacena ese valor, pero solo puede guardar hasta 6 decimales,
   por lo que redondea internamente:
   2026-06-17T21:32:52.791042Z      ← 6 decimales (microsegundos)
                              ↑
                    791041800 ns se redondeó a 791042 µs

3️⃣ Al consultar el registro inmediatamente después de crearlo,
   el valor que regresa la base de datos YA NO COINCIDE
   con el valor que Java generó originalmente
```

**¿Por qué esto es un problema real?**

Si tu API responde inmediatamente con el objeto recién creado usando el `Instant` que generó Java en memoria,
el cliente vería `createdAt` con 9 decimales. Pero si ese mismo cliente vuelve a consultar la cuenta con un
`GET /accounts/{accountNumber}`, el valor vendría de PostgreSQL con 6 decimales (y potencialmente redondeado). Aunque
representan el mismo instante físico, los strings JSON serían **diferentes**, lo cual puede causar comportamientos
inesperados en frontends que comparan fechas como texto, en tests automatizados que verifican igualdad exacta, o en
sistemas de caché que usan el timestamp como parte de una clave.

##### ✅ La solución: truncar antes de persistir

```bash
return () -> Optional.of(Instant.now().truncatedTo(ChronoUnit.MICROS));
```

`DateTimeProvider` es la interfaz de Spring Data que se invoca cada vez que se necesita generar el valor para
`@CreatedDate` o `@LastModifiedDate`. Al sobreescribirla con un `Bean` propio (referenciado mediante
`dateTimeProviderRef = "microsecondDateTimeProvider"`), forzamos a que Java **trunque** el `Instant` a microsegundos
**antes** de enviarlo a PostgreSQL:

```
1️⃣ Java genera y trunca el timestamp a microsegundos:
   2026-06-17T21:32:52.791042Z      ← ya truncado a 6 decimales

2️⃣ PostgreSQL almacena ese valor sin necesidad de redondear,
   porque ya coincide exactamente con su propia precisión nativa:
   2026-06-17T21:32:52.791042Z      ← idéntico

3️⃣ Al consultar el registro, el valor coincide exactamente con el que Java generó originalmente ✅
```

> 💡 **`truncatedTo` vs redondeo:** `truncatedTo(ChronoUnit.MICROS)` corta los decimales sobrantes (nanosegundos) sin
> redondear hacia arriba o abajo, simplemente los descarta. Esto es importante porque así garantizamos que el valor
> truncado en Java sea **exactamente** el mismo que PostgreSQL almacenará, sin depender de cómo PostgreSQL decida
> redondear internamente.

Con esta configuración, cualquier fecha que se genere automáticamente vía`@CreatedDate` o `@LastModifiedDate` mantiene
una precisión consistente entre la capa de aplicación (Java) y la capa de persistencia (PostgreSQL), evitando
inconsistencias al comparar o mostrar timestamps entre una operación de escritura y una posterior operación de lectura.
...
## 🧪 Tarea #7 — Probando endpoints del microservicio cuentas

### ✅ Probando el happy path de los endpoints
...
<
{
  "content": [
    {
      "accountNumber": "00219100230000000001",
      "accountType": "SAVINGS",
      "alias": "Mi cuenta de ahorros",
      "balance": 1500.00,
      "createdAt": "2026-06-17T21:19:06.121973Z",
      "currency": "PEN",
      "customerId": "CLI-00001",
      "status": "ACTIVE",
      "updatedAt": "2026-06-17T21:19:06.121973Z"
    },
    {...},
    {
      "accountNumber": "00219100230000000010",
      "accountType": "CHECKING",
      "alias": "Cuenta principal",
      "balance": 1100.00,
      "createdAt": "2026-06-17T21:19:06.121973Z",
      "currency": "PEN",
      "customerId": "CLI-00005",
      "status": "ACTIVE",
      "updatedAt": "2026-06-17T21:19:06.121973Z"
    }
  ],
...
- Listar cuentas enviando parámetros de paginación

````bash
$ curl -v -G --data "page=3&size=2" http://localhost:8080/api/v1/accounts | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 619
<
{
  "content": [
    {
      "accountNumber": "00219100230000000007",
      "accountType": "SAVINGS",
      "alias": "Fondo de emergencia",
      "balance": 4750.25,
      "createdAt": "2026-06-18T17:25:55.182716Z",
      "currency": "PEN",
      "customerId": "CLI-00004",
      "status": "ACTIVE",
      "updatedAt": "2026-06-18T17:25:55.182716Z"
    },
    {
      "accountNumber": "00219100230000000008",
      "accountType": "FIXED_TERM",
      "alias": "Inversión a plazo",
      "balance": 15000.00,
      "createdAt": "2026-06-18T17:25:55.182716Z",
      "currency": "USD",
      "customerId": "CLI-00004",
...
````bash
$ curl -v -G --data "customerId=CLI-00003" http://localhost:8080/api/v1/accounts | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 607
<
{
  "content": [
    {
      "accountNumber": "00219100230000000005",
      "accountType": "SAVINGS",
      "alias": "Ahorros en dólares",
      "balance": 2500.75,
      "createdAt": "2026-06-18T17:25:55.182716Z",
      "currency": "USD",
      "customerId": "CLI-00003",
      "status": "ACTIVE",
      "updatedAt": "2026-06-18T17:25:55.182716Z"
    },
    {
      "accountNumber": "00219100230000000006",
      "accountType": "CHECKING",
      "alias": "Cuenta cerrada",
      "balance": 0.00,
      "createdAt": "2026-06-18T17:25:55.182716Z",
      "currency": "PEN",
      "customerId": "CLI-00003",
...
- Listar todas las cuentas por status

````bash
$ curl -v -G --data "status=CLOSED" http://localhost:8080/api/v1/accounts | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 350
...
- Listar cuentas por cliente y status

````bash
$ curl -v -G --data "customerId=CLI-00003&status=ACTIVE" http://localhost:8080/api/v1/accounts | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 357
...
````bash
$ curl -v http://localhost:8080/api/v1/accounts/00219100230000000009 | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 239
<
{
  "accountNumber": "00219100230000000009",
  "accountType": "SAVINGS",
  "alias": null,
  "balance": 250.00,
  "createdAt": "2026-06-17T21:19:06.121973Z",
...
````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"customerId\": \"CLI-00001\", \"accountType\": \"SAVINGS\", \"currency\": \"PEN\"}" http://localhost:8080/api/v1/accounts | jq
>
...
<
{
  "accountNumber": "00219100230088532309",
  "accountType": "SAVINGS",
  "alias": null,
  "balance": 0.0,
  "createdAt": "2026-06-17T22:50:35.638325Z",
  "currency": "PEN",
  "customerId": "CLI-00001",
  "status": "ACTIVE",
  "updatedAt": "2026-06-17T22:50:35.638325Z"
}
````

- Actualizando alias de una cuenta

````bash
$ curl -v -X PUT -H "Content-Type: application/json" -d "{\"alias\": \"Mi ahorro 2026\"}" http://localhost:8080/api/v1/accounts/00219100230088532309 | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 248
<
{
  "accountNumber": "00219100230088532309",
  "accountType": "SAVINGS",
  "alias": "Mi ahorro 2026",
  "balance": 0.00,
  "createdAt": "2026-06-17T22:50:35.638325Z",
  "currency": "PEN",
  "customerId": "CLI-00001",
  "status": "ACTIVE",
  "updatedAt": "2026-06-17T22:51:23.869708Z"
}
````
...
````bash
$ curl -v -X DELETE http://localhost:8080/api/v1/accounts/00219100230000000003 | jq
>
...
````bash
$ curl -v -X PATCH -H "Content-Type: application/json" -d "{\"status\": \"ACTIVE\", \"reason\": \"Cumple condiciones para volver a activar\"}" http://localhost:8080/api/v1/accounts/00219100230000000009/status | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 238
<
{
  ...
  "customerId": "CLI-00005",
  "status": "ACTIVE",
  "updatedAt": "2026-06-17T21:50:52.291908Z"
}
````

### 🚫 Probando errores de cliente

- Busca un cliente que no existe

````bash
$ curl -v http://localhost:8080/api/v1/accounts/00219100230000000100 | jq
>
...
<
{
  "detail": "El número de cuenta [00219100230000000100] no se encuentra",
  "instance": "/api/v1/accounts/00219100230000000100",
  "status": 404,
  "title": "Not Found",
  "type": "https://banco-demo.example/errors/account-not-found",
  "timestamp": "2026-06-17T23:08:38.291882Z"
}
````

- Valida campos al registrar cuenta

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"customerId\": \"\", \"accountType\": \"SAVINGS\", \"currency\": \"PEN\"}" http://localhost:8080/api/v1/accounts | jq
>
...
<
{
  ...
  "title": "Bad Request",
  "type": "https://banco-demo.example/errors/validation-error",
  "timestamp": "2026-06-17T23:13:20.454556Z",
  "errors": [
    {
      "field": "customerId",
...
- Envía un accountType y currency no soportados

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"customerId\": \"CLI-00001\", \"accountType\": \"AHORRO\", \"currency\": \"SOLES\"}" http://localhost:8080/api/v1/accounts | jq
>
< HTTP/1.1 400 Bad Request
< Content-Type: application/problem+json
< Content-Length: 228
<
{
  ...
  "title": "Bad Request",
  "type": "https://banco-demo.example/errors/malformed-request",
  "timestamp": "2026-06-18T16:41:02.838145Z"
}
````

- Verifica máximo de cuentas permitidas por cliente
...
<
{
  "detail": "El cliente [CLI-00001] ya tiene el máximo de cuentas permitidas",
  "instance": "/api/v1/accounts",
  "status": 422,
  "title": "Unprocessable Content",
  "type": "https://banco-demo.example/errors/business-rule-violation",
  "timestamp": "2026-06-18T16:48:38.363806Z"
}
````

- Verifica que no se pueda modificar una cuenta con estado CLOSED

````bash
$ curl -v -X PATCH -H "Content-Type: application/json" -d "{\"status\": \"ACTIVE\", \"reason\": \"Volviendo a activar\"}" http://localhost:8080/api/v1/accounts/00219100230000000006/status | jq
>
< HTTP/1.1 422 Unprocessable Entity
< Content-Type: application/problem+json
< Content-Length: 272
...
{
  "detail": "No se puede activar una cuenta que está CLOSED",
  "instance": "/api/v1/accounts/00219100230000000006/status",
  "status": 422,
  "title": "Unprocessable Content",
  "type": "https://banco-demo.example/errors/business-rule-violation",
  "timestamp": "2026-06-18T16:52:15.797388Z"
}
````
...
````bash
$ curl -v -X PATCH -H "Content-Type: application/json" -d "{\"status\": \"CLOSED\", \"reason\": \"Bloqueando cuenta\"}" http://localhost:8080/api/v1/accounts/00219100230000000006/status | jq
>
...
<
{
  "detail": "La cuenta ya se encuentra en estado [CLOSED]",
  "instance": "/api/v1/accounts/00219100230000000006/status",
  "status": 409,
  "title": "Conflict",
  "type": "https://banco-demo.example/errors/invalid-status-transition",
  "timestamp": "2026-06-18T17:03:18.693553Z"
}
````

- Valida el patrón del Path Variable
...
<
{
  ...
  "title": "Bad Request",
  "type": "https://banco-demo.example/errors/validation-error",
  "timestamp": "2026-06-18T17:22:11.650107Z",
  "errors": [
    {
      "field": "accountNumber",
      "message": "must match \"^\\d{20}$\""
    }
  ]
}
````

## 🏦 Tarea #8 — Configurando y accediendo a Swagger UI

Con el versionado nativo de Spring Boot 4 ya configurado, surge un conflicto al intentar acceder a Swagger UI con su
ruta por defecto. En esta tarea documentamos el problema, el diagnóstico real (validado con pruebas) y la solución
aplicada.

### 🚨 El problema

Recordemos la configuración de versionado que ya tenemos en `application.yml`:
...
Y el controller, que declara explícitamente el placeholder `{version}` en su path:
...
Al intentar acceder a `Swagger UI` por su ruta por defecto:

```
http://localhost:8080/swagger-ui/index.html
```

La aplicación responde `500 Internal Server Error`, y en la consola del IDE aparece:
...
Spring registra varios `HandlerMapping` distintos, cada uno responsable de un tipo de ruta, con un orden de prioridad:
`RouterFunctionMapping` (rutas funcionales), `RequestMappingHandlerMapping` (métodos de `@RestController` anotados), y
un `HandlerMapping` específico para recursos estáticos.

El `ApiVersionStrategy` de Spring Boot 4 opera **dentro del pipeline de `RequestMappingHandlerMapping`**, que es el
componente que maneja exclusivamente las rutas de tus propios `@RestController`. Por eso:

| Ruta                                 | ¿Quién la maneja?                           | ¿Pasa por `RequestMappingHandlerMapping`?                                | ¿Aplica el versionado?                                        |
|--------------------------------------|---------------------------------------------|--------------------------------------------------------------------------|---------------------------------------------------------------|
| `/api/v1/accounts` o `/api/accounts` | `AccountController` (`@RestController`)     | ✅ Sí                                                                     | ✅ Sí — por eso falla si el segmento no es una versión válida  |
| `/v3/api-docs`                       | springdoc-openapi (registro interno propio) | ❌ No                                                                     | ❌ No                                                          |
| `/swagger-ui/**`                     | springdoc-openapi / recursos estáticos      | ❌ No, pero su mecanismo de **redirección interna** sí entra en conflicto | ⚠️ Falla por una causa relacionada, no por el mismo mecanismo |

En resumen: el versionado de Spring Boot 4 **aplica a cualquier ruta servida por un `@RestController`**, sin importar
si ese controller declaró `{version}` explícitamente o no, mientras la propiedad `path-segment` esté configurada
globalmente. Lo que queda fuera del versionado son las rutas que otros componentes (como springdoc-openapi) registran
por mecanismos distintos a `RequestMappingHandlerMapping`.

### ✅ La solución aplicada

Recordemos que la configuración por defecto de Swagger UI para el path, es:
...
Entonces, lo que haremos será alinear la ruta de Swagger UI con el mismo prefijo de versión que usa el resto de la API,
agregando esta configuración en `application.yml`:
...
Con este cambio, Swagger UI queda accesible en:
...
> Cuando se usa **path-based API versioning** (la versión como parte del path, como `/api/v1/...`),
> es consistente que toda la aplicación — incluyendo herramientas auxiliares como Swagger UI —
> viva bajo ese mismo namespace de rutas. Esto evita fricciones entre el mecanismo de versionado de
> tus controllers y las rutas de terceros como `springdoc-openapi`.
...
| Recurso                       | URL                                                  |
|-------------------------------|------------------------------------------------------|
| Swagger UI (interfaz gráfica) | `http://localhost:8080/api/v1/swagger-ui/index.html` |
| Especificación OpenAPI (JSON) | `http://localhost:8080/v3/api-docs`                  |

### 📖 Swagger-UI

![04.png](assets/webflux/04.png)

### 📄 api-docs

![05.png](assets/webflux/05.png)
